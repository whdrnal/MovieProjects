...
# 마이그레이트
docker exec python__2 bash -c "cd /data/site_projects/python__2/src/ ; python manage.py migrate --settings=config.settings.prod"

# 장고를 운영모드로 실행
docker exec python__2 bash -c "cd /data/site_projects/python__2/src ; nohup gunicorn --bind=0.0.0.0:8000 config.wsgi &"

# static collect 다시 수행
docker exec python__2 bash -c "cd /data/site_projects/python__2/src ; echo yes | python manage.py collectstatic --settings=config.settings.prod"
